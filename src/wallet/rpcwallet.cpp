...
static bool BetCastToBool(const std::vector<unsigned char>& vch)
{
...
static UniValue bid(const JSONRPCRequest& request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CWallet* const pwallet = wallet.get();
    ...
    if (request.fHelp || request.params.size() < 4 || request.params.size() > 5)
        throw std::runtime_error(
            RPCHelpMan{
                "bid",
                "\nBid an amount to a given address." +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The bid address to send to."},
                    {"amount", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The amount in " + CURRENCY_UNIT + " to bid. eg 0.1"},
                    {"hash1", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "hash1.\n"},
                    {"hash2", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "hash2\n"},
                    {"hash3", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "hash3\n"},
                   // {"hash4", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "hash4\n"},
                },
                RPCResult{
                    "\"txid\"                  (string) The transaction id.\n"},
                RPCExamples{
                    HelpExampleCli("bid", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 hash1 hash2 hash3 hash4")},
            }
                .ToString());

    std::string bidaddress = request.params[0].get_str(); //"myV5V3oQWJgzzxwp5suwSJM7HgU1zfsmRj"; 
    CTxDestination dest = DecodeDestination(bidaddress);
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    }
    ...
    UniValue input(UniValue::VType::VARR);
    UniValue outputs(UniValue::VType::VOBJ);
    outputs.pushKV(bidaddress, request.params[1]);
    UniValue lockTime;
    UniValue rbf;
    CMutableTransaction rawTx = ConstructTransaction(input, outputs, lockTime, rbf);

    /*
    // add flag tx out
    {
        CTxOut kFlag = rawTx.vout[0];

        // get last op code
        CScript kScript = kFlag.scriptPubKey;
        if (kScript.size() == 25 && kScript[0] == OP_DUP && kScript[1] == OP_HASH160 && kScript[2] == 20 && kScript[23] == OP_EQUALVERIFY && kScript[24] == OP_CHECKSIG) {
            //kScript[24] == OP_CHECKSIGVERIFY;
            kScript.back() = OP_CHECKSIGVERIFY;
        } else if (kScript.size() == CPubKey::PUBLIC_KEY_SIZE + 2 && kScript[0] == CPubKey::PUBLIC_KEY_SIZE && kScript.back() == OP_CHECKSIG) {
            kScript.back() = OP_CHECKSIGVERIFY;
        } else if (kScript.size() == CPubKey::COMPRESSED_PUBLIC_KEY_SIZE + 2 && kScript[0] == CPubKey::COMPRESSED_PUBLIC_KEY_SIZE && kScript.back() == OP_CHECKSIG) {
            kScript.back() = OP_CHECKSIGVERIFY;
        } else {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Only Support P2PKH or P2PK address");
        }
        //      if (kScript.IsPayToScriptHash()) {
        //          kScript.back() = OP_EQUALVERIFY;
        //      }
        //      if (kScript.IsPayToWitnessScriptHash()) {
        //	// to do
        //          kScript.clear();
        //}
        ...
        for (int i = 2; i < 6; ++i) {
            ...
            uint256 hash;
            hash.SetHex(request.params[i].get_str());
            std::vector<unsigned char> bytes = ToByteVector(hash);
            if (!BetCastToBool(bytes)) {
                std::string strFailReason = strprintf("Hash %s can't be zero or false", i - 1);
        ...
        //std::vector<unsigned char> kHash;
        //      kHash.insert(kHash.begin(), 12, (unsigned char)'a');
        //      kScript << kHash;


        kFlag.scriptPubKey = kScript;
        kFlag.nValue /= 1000;
        while (IsDust(kFlag, ::dustRelayFee)) {
            kFlag.nValue *= 2;
        }

        rawTx.vout.push_back(kFlag);
    }
    */

    //push bid data
    CScript kDataScript;
    kDataScript << OP_RETURN;
    for (int i = 2; i < 5; ++i) {
        if (request.params[i].isNull()) {
            break;
        }
    ...
    if (kDataScript.size() > nMaxDatacarrierBytes) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "OP_RETURN size overload!");
    }

    CTxOut kFlag;
    kFlag.scriptPubKey = kDataScript;
    kFlag.nValue = DUST_RELAY_TX_FEE;
    while (IsDust(kFlag, ::dustRelayFee)) {
        kFlag.nValue *= 2;
    }
    rawTx.vout.push_back(kFlag);
    ...
    // Sign the transaction
    {
        ...
        UniValue preTxs;
        UniValue hashType;
        UniValue rawHash = SignTransaction(pwallet->chain(), rawTx, preTxs, pwallet, false, hashType);
        //return rawHash;
    }

    {
        // parse hex string from parameter
        CMutableTransaction& mtx = rawTx;
...
static void FindBidHashs(CTransactionRef tx, UniValue& kHashs)
{
    for (int z = 0; z < tx->vout.size(); ++z) {
        const CTxOut& otest = tx->vout[z];
        const CScript& kScript = otest.scriptPubKey;

        //CScript::const_iterator pc = kScript.begin();
        //opcodetype opcode;
        //if (kScript.size() > 25 && kScript[0] == OP_DUP && kScript[1] == OP_HASH160 && kScript[2] == 20 && kScript[23] == OP_EQUALVERIFY && kScript[24] == OP_CHECKSIGVERIFY) {
        //    pc += 25;
        //} else if (kScript.size() > CPubKey::PUBLIC_KEY_SIZE + 2 && kScript[0] == CPubKey::PUBLIC_KEY_SIZE && kScript.back() == OP_CHECKSIGVERIFY) {
        //    pc += CPubKey::PUBLIC_KEY_SIZE;
        //} else if (kScript.size() > CPubKey::COMPRESSED_PUBLIC_KEY_SIZE + 2 && kScript[0] == CPubKey::COMPRESSED_PUBLIC_KEY_SIZE && kScript.back() == OP_CHECKSIGVERIFY) {
        //    pc += CPubKey::COMPRESSED_PUBLIC_KEY_SIZE;
        //}
        //if (pc > kScript.begin()) {

            /*
            std::vector<unsigned char> vchRet;
            kScript.GetOp(pc, opcode, vchRet);

            uint256 hash1(vchRet);
            kHashs.push_back(hash1.GetHex());
            //result.pushKV("Attached Hash1", hash1.GetHex());

            vchRet.clear();
            kScript.GetOp(pc, opcode, vchRet);

            uint256 hash2(vchRet);
            kHashs.push_back(hash2.GetHex());
			*/
        if (kScript.size() >= 1 && kScript[0] == OP_RETURN) {
            CScript::const_iterator pc = kScript.begin() + 1;
            opcodetype opcode;
            std::vector<unsigned char> vchRet;
            while (kScript.GetOp(pc, opcode, vchRet)) {
                if (opcode <= OP_PUSHDATA4 && vchRet.size() > 0 ) {
                    if (vchRet.size() != 32) { // hash3
                        vchRet.resize(32, 0);
                    }
                    uint256 hash(vchRet);
                    kHashs.push_back(hash.GetHex());
                    vchRet.clear();
				}
            }
            break;
            //result.pushKV("Attached Hash2", hash2.GetHex());
        }
    }
}

static UniValue getBid(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            RPCHelpMan{
                ...
                "\nGet Bid information. \n",
                {
                    {"block height", RPCArg::Type::STR, RPCArg::Optional::NO, "The block height stored the information."},
                    {"bid address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to bid."},
                    {"block head hash count", RPCArg::Type::STR, RPCArg::Optional::NO, "The amount of block head hash."},
                },
                RPCResult{
                    "\"info\"                  (string) The bid information.\n"},
                RPCExamples{
                    HelpExampleCli("getbid", "100 , \"myV5V3oQWJgzzxwp5suwSJM7HgU1zfsmRj\", 10"),
                },
            }
            .ToString());

    int iHeight = request.params[0].get_int();
    if (iHeight <= 0) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "block height error");
    }
    ...
    UniValue betList(UniValue::VARR);
    for (int i = 0; i < block.vtx.size(); ++i) {
        CTransactionRef tx = block.vtx[i];
        for (int j = 0; j < tx->vout.size(); ++j) {
            const CTxOut& out = tx->vout[j];
            if (out.scriptPubKey == scriptPubKey) {
                UniValue betoj(UniValue::VOBJ);
                betoj.pushKV("amount", out.nValue);
                UniValue kHashs(UniValue::VARR);
                FindBidHashs(tx, kHashs);
                betoj.pushKV("hashs", kHashs);
                betList.push_back(betoj);
                break;
            }
        }
    }
    ...
    if (request.params.size() > 2) {
        iHashCount = request.params[2].get_int();
    }
    UniValue blockList(UniValue::VARR);
    for (int i = 0; i < iHashCount; ++i) {
        CBlockIndex* pblockindex = chainActive[iHeight + i];
        if (pblockindex == nullptr) {
            std::string strError = strprintf("Error getBid no more block hash for height %d, i/iHashCount %d/%d\n", iHeight + i, i, iHashCount);
            LogPrintf(strError.c_str());
            throw JSONRPCError(RPC_VERIFY_ERROR, strError);
        }
        blockList.push_back(pblockindex->GetBlockHash().GetHex());
    }
    result.pushKV("hashes", blockList);
    result.pushKV("bids", betList);
...
{ //  category              name                                actor (function)                argNames
    ...
    { "wallet",             "bid",                &bid,             {"address","amount","bethash","lockhead"} },
    { "wallet",             "getbid",                &getBid,             {"height","address","hash count"} },
};
// clang-format on
